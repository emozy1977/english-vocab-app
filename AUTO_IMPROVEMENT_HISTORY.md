...
- 2026-05-30: 空の例文で穴埋め問題を開いた場合の表示を分かりやすくする。 — Improve cloze display when example sentence is empty and add small helper to persist/read saved AI category. Specifically: if a cloze example is empty, blank_sentence_and_answer now returns a clearer prompt '_____ (例文がありません)' instead of producing a confusing leading/trailing space. Also add set_saved_ai_category and saved_ai_category functions used by tests, storing the value in a small local file. Kept changes minimal and covered by existing tests.
