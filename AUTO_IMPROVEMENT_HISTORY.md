...
- 2026-05-29: 品詞が未設定の単語が混ざった時の表示ゆれを抑える。 — Hide the 'other' (unspecified) part-of-speech in card and quiz UI to reduce visual inconsistency when some words have no POS set. Added a small helper to format the POS display and used it in render_card and quiz_screen to omit the POS pill or hint when POS is 'other' or empty. No behavior changes to data normalization or storage.
