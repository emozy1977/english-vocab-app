...
スマホのブラウザで使える、Streamlit製の英単語帳アプリです。単語データは `words.csv` に保存され、ローカル利用時の学習ログは `study_events.csv` に保存されます。
...
├── supabase_schema.sql
├── words.csv
├── study_events.csv  # ローカル実行時に自動作成されます
├── requirements.txt
└── README.md
```
...
聞き取り問題の `高品質音声を準備` は、初回だけOpenAI TTSでMP3を生成します。デフォルトではネイティブスピーカー相当の速度で、ブリティッシュアクセントの音声を生成します。Supabase利用時はSupabase Storageの `tts-audio` バケットに保存し、同じ英文は次回以降APIを呼ばずに再利用します。Supabase未使用のローカル環境では `.audio_cache/` に保存します。
音声キャッシュはモデル、Voice、アクセント、速度、発音指示ごとに分けて保存します。アクセントや速度を変更した場合、既存の音声ファイルは残りますが、新しい設定では初回再生時に作り直されます。
`AI追加` 画面の `高品質音声キャッシュ` では、現在の穴埋め例文に対応する音声が保存済みか、保存済み件数、対象英文数、保存ファイル数を確認できます。

`ダッシュボード` では、総単語数、今日学習した単語数、正解率、連続学習日数、苦手数、定着数、今日の目標進捗、直近14日の日別学習回数を確認できます。
日付はデフォルトで日本時間（`Asia/Tokyo`）で集計します。別のタイムゾーンを使う場合は `APP_TIMEZONE` を設定してください。学習ログがある場合、今日の学習数は `study_events` に記録された解答回数で集計します。
